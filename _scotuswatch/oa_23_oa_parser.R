###############################################################################
#OA Parser
#Code Developed Using scotustext(R) by Jake S. Truscott
###############################################################################

{

  {
    path <- "C:/Users/Jake Truscott/Documents/GitHub/jaketruscott.github.io/_scotuswatch/ot23_arguments/October Sitting"

    begin <- Sys.time()
    scotus_OT23_October_23 <- scotustext::oa_parser(dir_path = path)
    end <- Sys.time()
    compilation <- end - begin
    message("\nCompilation Time: ", compilation, ' Seconds')

    scotus_OT23_October_23 <- scotus_OT23_October_23 %>%
      mutate(sitting = 'October')

  } #October Sitting Arguments

  {
    path <- "C:/Users/Jake Truscott/Documents/GitHub/jaketruscott.github.io/_scotuswatch/ot23_arguments/November Sitting"

    begin <- Sys.time()
    scotus_OT23_November_23 <- scotustext::oa_parser(dir_path = path)
    end <- Sys.time()
    compilation <- end - begin
    message("\nCompilation Time: ", compilation, ' Seconds')

    scotus_OT23_November_23 <- scotus_OT23_November_23 %>%
      mutate(sitting = 'November')

  } #November Sitting Arguments

  {
    path <- "C:/Users/Jake Truscott/Documents/GitHub/jaketruscott.github.io/_scotuswatch/ot23_arguments/December Sitting"

    begin <- Sys.time()
    scotus_OT23_December_23 <- scotustext::oa_parser(dir_path = path)
    end <- Sys.time()
    compilation <- end - begin
    message("\nCompilation Time: ", compilation, ' Seconds')

    scotus_OT23_December_23 <- scotus_OT23_December_23 %>%
      mutate(sitting = 'December') %>%
      mutate(case_name = ifelse(argument == '22-6389', 'Brown v. United States', case_name))

  } #December Sitting Arguments

  scotus_OT23 <- rbind(scotus_OT23_October_23,
                       scotus_OT23_November_23,
                       scotus_OT23_December_23) %>%
    mutate(case_name = ifelse(argument == '22-6389', 'Brown v. United States', case_name)) %>%
    mutate(speaker = ifelse(speaker == "MS. SANTO" & argument == '22-666', 'MS. SANTOS', speaker)) %>%
    mutate(speaker = ifelse(speaker == 'JUSTICE PRELOGAR', 'GENERAL PRELOGAR', speaker)) %>%
    mutate(speaker_type = ifelse(speaker == 'GENERAL PRELOGAR' & speaker_type == "Justice", "Attorney", speaker_type))
  ...
  #Don't Forget to Push to Github Before Doing Stats!

} #Old Code Using SCOTUSTEXT
